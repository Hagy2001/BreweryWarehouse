...
All three button variants share identical geometry: `border-radius: 4px`, padding `6px 16px`, `font-family: "Barlow Condensed"`, `font-size: 13px`, `text-transform: uppercase`. They differ only in colour.

| Class | Visual | When to use |
|---|---|---|
| `bw-btn bw-btn-primary` | Amber fill, dark text | Confirm, Save, Submit, Create — the one positive action per form |
| `bw-btn bw-btn-ghost` | Transparent + border | Cancel, Back, secondary navigation, Logout |
| `bw-btn bw-btn-danger` | Red fill, white text | Delete, destructive / irreversible actions only |

**Usage rules (apply app-wide):**
- Every form that mutates data must pair exactly one `bw-btn-primary` (confirm) with one `bw-btn-ghost` (cancel/back).
- Delete actions get a `bw-btn-danger` confirm button; place it after the primary action if both appear together (Details pages: Edit primary → Delete danger).
- Logout in the sidebar uses `bw-btn bw-btn-ghost` — it is a navigation action, not a destructive one.
- Never swap colours: amber is never used for delete; red is never used for save.
- Auth pages use `.bw-login-btn` (full-width amber) instead of `bw-btn-primary` — do not mix them.
